...
void Logic::restoreCompositeState()
{
    auto command = std::make_shared<RestoreCompositeStateCommand>();
...
void Logic::saveCompositeState()
{
    auto command = std::make_shared<SaveCompositeStateCommand>();

    facade.execute(command);
}

void Logic::make_composit(std::vector<int> &idx)
{
    auto command = std::make_shared<SceneUnionComp>(idx);
    facade.execute(command);   
}


void Logic::moveComposit(float dx, float dy, float dz)
{
    auto offset = Vector3{dx, dy, dz};

    saveCompositeState();

    auto command = std::make_shared<CompositeMoveCommand>(offset);
    facade.execute(command);
...
void Logic::rotateComposit(float dx, float dy, float dz)
{
    auto offset = Vector3{dx, dy, dz};

    saveCompositeState();

    auto command = std::make_shared<CompositeRotateCommand>(offset);
    facade.execute(command);

    draw();
}

void Logic::scaleComposit(float dx, float dy, float dz)
{
    auto offset = Vector3{dx, dy, dz};

    saveCompositeState();

    auto command = std::make_shared<CompositeScaleCommand>(offset);  
    facade.execute(command);
