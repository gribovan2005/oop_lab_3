...
#include "ModelSourceFactoryCreator.h"
#include <filesystem>
    ...
    ui(new Ui::MainWindow)
{
    ...
    meta = std::make_shared<MetaData>(); // поиенять в лоджик
...
void MainWindow::setupScene(QWidget *parent)
{
    ...
    solution.registrate(".ves", []() {
        return ModelSourceFactoryCreatorMaker::create<VertexEdgeSourceFactory>()->createFactory();
    });

    solution.registrate(".adls", []() {
        return ModelSourceFactoryCreatorMaker::create<AdjacencyListSourceFactory>()->createFactory();
    });

    solution.registrate(".csva", []() {
        return ModelSourceFactoryCreatorMaker::create<CSVAdjencySourceFactory>()->createFactory();
    });

    solution.registrate(".csve", []() {
        return ModelSourceFactoryCreatorMaker::create<CSVEdgeSourceFactory>()->createFactory();
    });
...
void MainWindow::connects()
{
    ...
    connect(ui->btn_composit, &QPushButton::clicked, this, 
                &MainWindow::btnCompositClicked);
    

    connect(ui->btn_move_composit, &QPushButton::clicked, this,
            &MainWindow::btnMoveCompositClicked);
    connect(ui->btn_rotate_composit, &QPushButton::clicked, this,
            &MainWindow::btnRotateCompositClicked);
    connect(ui->btn_scale_composit, &QPushButton::clicked, this,
            &MainWindow::btnScaleCompositClicked);

    connect(ui->btn_undo_composite, &QPushButton::clicked, this, 
                &MainWindow::btnUndoCompositeClicked);
...
void MainWindow::btnNewCameraClicked()
{
    ...
    item->setData(Qt::UserRole + 1, "camera");
    ui->list_composit->addItem(item);
}
...
void MainWindow::btnDeleteCameraClicked()
{
    ...
    int deletedCameraId = ui->cameraCB->currentData().toInt();

    logic->deleteCamera(deletedCameraId);

    ui->cameraCB->removeItem(ui->cameraCB->currentIndex());

    logic->setActiveCamera(ui->cameraCB->currentData().toInt());

    for (int i = 0; i < ui->list_composit->count(); ++i)
    {
        QListWidgetItem *item = ui->list_composit->item(i);
        if (item->data(Qt::UserRole).toInt() == deletedCameraId)
        {
            delete ui->list_composit->takeItem(i);
            break;
        }
    }
...
void MainWindow::btnLoadModelClicked()
{
    ...
    QListWidgetItem *item = new QListWidgetItem(figName);
...
void MainWindow::btnDeleteModelClicked()
{
    ...
    int objectId = ui->modelCB->currentData().toInt();

    try
    {
        logic->deleteObject(objectId);
    }
    catch (const BaseException &error)
    {
    ...
    {
        QListWidgetItem *item = ui->list_composit->item(i);
        if (item->data(Qt::UserRole).toInt() == objectId)
        {
            delete ui->list_composit->takeItem(i);
            break;
        }
...
void MainWindow::btnDeleteAllModelClicked()
{
    ...
    for (int i = ui->list_composit->count() - 1; i >= 0; --i)
    {
        QListWidgetItem *item = ui->list_composit->item(i);
...
void MainWindow::btnUndoCompositeClicked()
{
    if (ui->modelCB->count() == 0)
    {
        showError("Список объектов пуст!");
    ...
    logic->restoreCompositeState();
}


void MainWindow::btnCompositClicked()
{
    std::vector<int> selectedIndices;

    QList<QListWidgetItem *> selectedItems = ui->list_composit->selectedItems();
    ...
    qDebug() << selectedIndices;
    logic->make_composit(selectedIndices);
}

void MainWindow::btnMoveCompositClicked()
{
    bool dx_ok, dy_ok, dz_ok;
    float dx, dy, dz;
    ...
    logic->moveComposit(dx, dy, dz);
}

void MainWindow::btnRotateCompositClicked()
{
    bool ax_ok, ay_ok, az_ok;
    float ax, ay, az;
    ...
    logic->rotateComposit(ax, ay, az);
}

void MainWindow::btnScaleCompositClicked()
{
    bool kx_ok, ky_ok, kz_ok;
    float kx, ky, kz;
